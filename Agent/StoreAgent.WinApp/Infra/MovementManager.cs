...
public class MovementManager
{
    ...
    private const int NEARPLANE_3_OFFSET = 0x6D6E7C; // Novo offset para NEARPLANE_3

    private readonly MemoryManager _memoryManager;
    private readonly PacketManager _packetManager;

    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate void MoveFunctionDelegate(IntPtr playerPtr, int x, int y);
    ...
    public bool MoveTo(int x, int y)
    {
        try
        {
            ...
            // Envia o pacote de movimento para o servidor
    ...
    public bool NativeMoveTo(int x, int y)
    {
        try
        {
            var process = _memoryManager.GetProcess();
            ...
            var baseAddress = process.MainModule.BaseAddress;
            var playerPtr = GetPlayerPointer();

            if (playerPtr == IntPtr.Zero)
            {
                Console.WriteLine("Ponteiro do jogador não encontrado.");
                return false;
            }

            // Obtém o endereço da função de movimento
            var moveFunctionAddress = IntPtr.Add(baseAddress, MOVE_FUNCTION_OFFSET);

            // Cria o delegate para chamar a função
            var moveFunction = Marshal.GetDelegateForFunctionPointer<MoveFunctionDelegate>(moveFunctionAddress);

            // Chama a função de movimento nativa
            moveFunction(playerPtr, x, y);

            Console.WriteLine($"Chamando função nativa de movimento para X={x}, Y={y}");
            return true;
        }
        catch (Exception ex)
        {
            Console.WriteLine($"Erro ao chamar função nativa de movimento: {ex.Message}");
            Console.WriteLine($"Stack trace: {ex.StackTrace}");
            return false;
        }
    }
